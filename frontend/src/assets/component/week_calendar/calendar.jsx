...
const WeekCalendar = (props) => {
  ...
  React.useEffect(() => {
    setAgendamentosDaSala(
      reservaDAO.getAgendamentosFromSala(props.agendamentos, props.sala)
    );
  ...
  const organizaAgendamentos = (agendamentos) => {
    ...
    agendamentos.forEach((agendamento) => {
      ...
      if (dataString in hash) {
        hash[dataString] = hash[dataString] = [
          ...hash[dataString],
          agendamento,
        ];
      } else {
        ...
        if (i + 1 < actualArray.length) {
          if (actualArray[i].hora_fim === actualArray[i + 1].hora_inicio) {
            ...
            if (horaInicio === 0 && horaFim === 0) {
              [horaInicio, horaFim] = [
                actualArray[i].hora_inicio,
                actualArray[i].hora_fim,
              ];
            }
    ...
    for (let bloqueio of bloqueios) {
      if (Array.isArray(bloqueio.sala)) {
        for (let currentSala of bloqueio.sala) {
          if (
            currentSala.toString() === sala._id.toString() &&
            moment(date)
              .subtract(1, 'day')
              .isSame(new Date(bloqueio.dia), 'day') &&
            bloqueio.wholeDay
          ) {
            return true;
          }
        ...
        if (
          bloqueio.sala.toString() === sala._id.toString() &&
  ...
  return (
    <div className={'container_week'}>
      <div
        ...
        className={
          selectedDate.isSame(new Date(), 'week') ? 'chevron hidden' : 'chevron'
        }
      >
        <p>
      ...
      {days.map((day, index) => {
        let date = moment(selectedDate.toDate())
          .locale('pt-BR')
          .startOf('week')
          .add(index, 'days');
        ...
        return (
          <div key={day} className={'week_day'}>
            <h1 className={date.isSame(new Date(), 'day') ? 'today' : ''}>
              {day}
            </h1>
            <h3 className={date.isSame(new Date(), 'day') ? 'today' : ''}>
              {date.format('DD/MMM')}
            </h3>
            {agendamentosDaSala.map((agendamento, index) => {
              if (
                date.isSame(agendamento.data, 'day') &&
                !agendamento.cancelado
              ) {
                if ('mes' in agendamento) {
                  if (date.isSame(new Date(), 'day')) {
                    return !props.fetchedAgendamentos ? (
                      <p>Carregando...</p>
                    ) : (
                      <div
            ...
            {date.isSameOrAfter(new Date(), 'day') ? (
              verificarBloqueio(date, props.sala, props.bloqueiosSalas) ? (
                <div
                  style={{ backgroundColor: 'transparent', display: 'flex' }}
                >
                  <i style={{ color: '#CCC', margin: 'auto' }}>
                    Sala Indisponível
                  </i>
                </div>
              ) : !props.fetchedAgendamentos ? (
                <p>Carregando...</p>
              ) : (
                <div
                  onClick={() => {
      ...
      <div
        ...
        className={
          selectedDate.isSame(moment().add(2, 'week'), 'week')
            ? 'chevron hidden'
            : 'chevron'
        }
      >
