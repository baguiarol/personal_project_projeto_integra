...
const Sala = (props) => {
  ...
  const verificaAluguelMensal = () => {
    for (let i = 0; i < props.agendamentos.length; i++) {
      if ('mes' in props.agendamentos[i]) {
        if (
          props.agendamentos[i].sala_id.toString() ===
            props.sala._id.toString() &&
  ...
  React.useCallback(() => {
    if (props.userLogged.sala_fixa && '_id' in props.sala) {
      setSalaFixa(
        props.sala._id.toString() === props.userLogged.sala_fixa.toString()
      );
    }
  ...
  return (
    <div className={'sala_content'}>
        ...
        <div>
          <h1>
            <span
              ...
              onClick={async () => {
                if (salaFixa) {
                  await clienteDAO.fixarSalaNoTopo(
                    props.sala,
                    props.userLogged
                  );
                  ...
                  props.setUserLogged({
                    ...props.userLogged,
                    sala_fixa: props.sala._id,
                  });
                  setSalaFixa(true);
                }
              }}
              data-for={'main'}
              data-tip={'Fixar no Topo'}
              data-iscapture={'true'}
            >
      ...
      {aluguelMensal ? (
        <h2 className={'aluguel_mensal'}>
          Esta sala está alugada por todo o mês e não está disponível para
          agendamentos.
        </h2>
      ) : (
        <WeekCalendar
          sala={props.sala}
          isAdm={props.isAdm}
          addReservaListener={props.addReservaListener}
        />
      )}
    </div>
  );
