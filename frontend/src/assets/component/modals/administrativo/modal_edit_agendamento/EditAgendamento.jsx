...
const ModalEditAgendamento = props => {
    ...
    React.useEffect(() => {
        if (props.agendamentos.length > 0 && '_id' in props.agendamentoSelected) {
            props.selectAgendamentos(reservaDAO.getAgendamentosById(props.agendamentos, props.agendamentoSelected._id))
        }
    }, [props.agendamentos])
                ...
                <ModalParent
                    ...
                    body={<div className={'body_edit_agendamento'}>
                        ...
                        <Button
                            ...
                            onClick={async () => {
                                if (editarSala) {
                                    ...
                                    showToastLoading('Carregando: Trocando '+props.agendamentoSelected.profissional.nome+' de sala')
                                    await logDAO.create({usuario: props.userLogged,
                                        log: `Trocou sala da reserva ${props.agendamentoSelected.profissional.nome} ${moment(props.agendamentoSelected.data).format("DD-MM-YYYY")} ${props.agendamentoSelected.hora_inicio}h-${props.agendamentoSelected.hora_fim}h ${props.agendamentoSelected.sala.nome} para sala ${editarSala.label}`,
                                        data_hora: new Date()})
                                    await reservaDAO.editaReserva(props.agendamentoSelected._id,{ sala_id: editarSala.id});
                                    showToastSuccess('Sucesso na troca da '+props.agendamentoSelected.profissional.nome+' de sala')
                                }
                            }}/>
                        <h2>Execução</h2>
                        <h3>
                                ...
                                finalizado ? <></> :
                                    <div style={{display: 'flex'}}>
                                        <Button
                                            onClick={async () => {
                                                if (executing) {
                                                    showToastLoading('Carregando: Finalizando reserva de '+props.agendamentoSelected.profissional.nome);
                                                    await reservaDAO.executaReserva(props.agendamentoSelected._id);
                                                    await updateAgendamentos();
                                                    showToastSuccess('Reserva de '+props.agendamentoSelected.profissional.nome+' finalizada!');

                                                } else {
                                                    showToastLoading('Carregando: Iniciando reserva de '+props.agendamentoSelected.profissional.nome);
                                                    await reservaDAO.comecaReserva(props.agendamentoSelected._id);
                                                    await updateAgendamentos();
                                                    showToastSuccess('Reserva de '+props.agendamentoSelected.profissional.nome+' iniciada!');
                                                }
                                            }}
                                            type={'button'}
                        ...
                        <div>
                                    ...
                                    onClick={async () => {
                                        if (cancelamentoData.reservaInteira) {
                                            if (window.confirm('Tem certeza que deseja cancelar toda a reserva?')) {
                                                showToastLoading('Carregando: Cancelamento da Reserva de: '+props.agendamentoSelected.profissional.nome);
                                                props.close()
                                                await reservaDAO.cancelaReserva(props.agendamentoSelected, props.userLogged)
                                                props.selectAgendamentos({});
                                                showToastSuccess('Cancelamento da Reserva de '+props.agendamentoSelected.profissional.nome+' efetuada!');
                                            }
                                        } else {
                                            if (window.confirm(
                                                `Tem certeza que deseja cancelar a reserva das ${cancelamentoData.hora_inicio}h até às ${cancelamentoData.hora_fim}h`)) {
                                                try {
                                                    showToastLoading('Carregando: Cancelamento Parcial da Reserva de: '+props.agendamentoSelected.profissional.nome);
                                                    props.close()
                                                    let aux = {...props.agendamentoSelected};
                                                    props.selectAgendamentos({});
                                                    await reservaDAO
                                                        .cancelaParteDaReserva(aux._id,
                                                            cancelamentoData.hora_inicio,
                                                            cancelamentoData.hora_fim,
                                                            props.agendamentos,
                                                            props.userLogged);
                                                    showToastSuccess('Cancelamento Parcial da Reserva de '+aux.profissional.nome+' efetuada!');
                                                } catch (e) {
                                                    alert(e);
                                                }
                                                props.close()
                            ...
                            <Button text={"Excluir Reserva"} width={'45%'} onClick={async () => {
                                if (window.confirm("Tem certeza que deseja excluir a reserva?")) {
                                    showToastLoading('Carregando: Excluindo Reserva de: '+props.agendamentoSelected.profissional.nome);
                                    await logDAO.create({usuario: props.userLogged,
                                        log: `Excluiu reserva ${props.agendamentoSelected.profissional.nome} ${moment(props.agendamentoSelected.data).format("DD-MM-YYYY")} ${props.agendamentoSelected.hora_inicio}h-${props.agendamentoSelected.hora_fim}h ${props.agendamentoSelected.sala.nome}`,
                                        data_hora: new Date()})
                                    await reservaDAO.delete({_id: props.agendamentoSelected._id});
                                    showToastSuccess('Reserva de: '+props.agendamentoSelected.profissional.nome+' excluída!');
                                }
                            }}/>
                        </div>
                        <h2>Pagamento</h2>
                        <h3>
                            ...
                            {pago ? <></>
                                : <Button
                                    onClick={async () => {
                                        showToastLoading('Carregando: Pagamento da Reserva de: '+props.agendamentoSelected.profissional.nome);
                                        await reservaDAO.pagaReserva(props.agendamentoSelected._id, true, props.userLogged,);
                                        await updateAgendamentos();
                                        showToastSuccess('Reserva de: '+props.agendamentoSelected.profissional.nome+' paga!');
                                    }}
                                    loading={loading}
                                    type={'button'}
                                    text={'Pagar'}
