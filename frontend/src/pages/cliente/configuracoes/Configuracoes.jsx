...
const Configuracoes = props => {
    ...
    const options = ['Nutricionista', 'Psicólogo', 'Fonoaudiólogo', 'Fisioterapeuta',
        'Acupunturista', 'Psicopedagogo', 'Psicanalista', 'Terapeuta',
        'Farmacêutico', 'Médico', 'Psiquiatra'];

    const generateOptions = () => (options.map(option => ({label: option, value: option})));

    React.useEffect(() => {
        console.log('Usuário Logado', props.userLogged);
    }, [props.userLogged]);
        ...
        <div className={'configuracoes_container'}>
            ...
            <div className={'configuracoes_body'}>
                <form onSubmit={onSubmitDados}>
                    <div style={{display: 'flex', width: '100%', alignItems: "flex-end", marginTop: 35}}>
                        <div>
                            <div
                        ...
                        <InputText
                            required
                            name={'dataNascimento'}
                            defaultValue={moment(new Date(props.userLogged.dataNascimento)).format('YYYY-MM-DD')}
                            type={'date'}
                            style={{flexGrow: 0.5, marginRight: 15}} label={'Data de Nascimento'}/>
                        <LabelSelect label={'Profissão'} style={{flexGrow: 1, marginRight: 15}}
                                     options={generateOptions()}/>
                        <InputText
                            required
                            name={'registro'}
                            defaultValue={props.userLogged.registro}
                            label={'Nº de Registro'} style={{flexGrow: 0.3, marginRight: 15}}/>
                    </div>
                    <div style={{display: 'flex', width: '100%', marginTop: 35}}>
                        <InputText
                            ...
                            style={{flexGrow: 0.5, marginRight: 15}} label={'CPF'}/>
                        <InputText
                            required
                            name={'telefone'}
                            defaultValue={props.userLogged.telefone}
                            style={{flexGrow: 0.5, marginRight: 15}} label={'Telefone'}/>
                        <InputText
                            name={'endereco'}
                            required
                            defaultValue={props.userLogged.endereco}
                            style={{flexGrow: 2, marginRight: 15}} label={'Endereço'}/>
                        <InputText
                            name={'cep'}
                            required
                            defaultValue={props.userLogged.cep}
                            label={'CEP'} style={{flexGrow: 0.5, marginRight: 15}}/>
                    </div>
                    <h2>Alterar Descrição</h2>
                    <p>Sua descrição é seu currículo, você pode incluir o que você faz, sua formação acadêmica, e quanto
                        tempo de experiência na área que você atua.</p>
                    <p><textarea name={'descricao'}
                                 defaultValue={props.userLogged.descricao}
                                 required placeholder={'Sua nova descrição vai aqui.'}
                                 style={{width: '50%', height: 150, padding: 15}}/></p>
